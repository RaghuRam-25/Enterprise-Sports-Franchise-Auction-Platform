    ...
    <div className="space-y-6">
            ...
            return (
              <button
                ...
                className={`px-4 py-2 rounded-xl text-xs font-bold transition flex items-center gap-2 whitespace-nowrap ${
                  isTabActive
                    ? 'bg-[#0B2B26] text-white shadow-lg font-black border border-[#0B2B26]'
                    : 'bg-[#151515] text-[#F5F5F5] border border-[#333333] hover:border-[#0B2B26] hover:text-white'
                }`}
              >
                <span>{tab.label}</span>
                {isStageCurrent && (
                  <span className="w-2 h-2 rounded-full bg-white animate-pulse" title="Active Platform Stage" />
                )}
              </button>
            );
          })}
            ...
            return (
              ...
              >
                <div className="flex items-center justify-between">
                  <span className={`text-[10px] font-mono font-bold ${isCurrent ? 'text-white' : isDone ? 'text-white' : 'text-mutedText'}`}>
                    {String(i + 1).padStart(2, '0')}
                  </span>
                  {isCurrent && <span className="text-[9px] font-bold uppercase text-white tracking-wider">Active</span>}
                  {isDone && <span className="text-[9px] font-bold uppercase text-white tracking-wider">Passed</span>}
                </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div className="bg-darkBg/60 border border-cardBorder/80 p-3.5 rounded-2xl space-y-3">
              <div className="flex items-center justify-between border-b border-cardBorder/60 pb-2">
                <span className="text-xs font-black text-white uppercase tracking-wider">1. Registration</span>
              </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="glass-card rounded-2xl p-5 border border-cardBorder relative overflow-hidden">
          <div className="flex justify-between items-start">
            ...
            <div className="p-3 bg-neonGreen/10 text-white rounded-xl border border-neonGreen/20">
      ...
      <div className="glass-card rounded-2xl p-6 border border-cardBorder space-y-4">
        <div className="flex justify-between items-center">
          ...
          <Link
            to="/admin/teams"
            className="text-xs text-white hover:text-white font-semibold flex items-center gap-1"
          >
            Manage Teams &rarr;
          </Link>
        </div>
        ...
        {teams.length === 0 ? (
          ...
          <div className="grid gap-2.5">
            {teams.map(team => {
              ...
              const brandColor = getTeamTheme(team)?.primaryColor || '#0B2B26';
                ...
                >
                  ...
                  <div className="min-w-0 flex-1 basis-[180px]">
                    ...
                    <div className="mt-2.5 flex items-center gap-2 max-w-[240px]">
                      <Users className={`w-3.5 h-3.5 shrink-0 ${rosterComplete ? 'text-white' : 'text-mutedText'}`} />
                      <div className="h-1.5 w-full bg-darkBg/80 border border-cardBorder/60 rounded-full overflow-hidden">
                        <div
                          className={`h-full rounded-full transition-all duration-500 ${rosterComplete ? 'bg-primaryText' : rosterPct >= 50 ? 'bg-secondaryText' : 'bg-warningGold'}`}
                          style={{ width: `${Math.max(rosterPct, 6)}%` }}
                        />
                      </div>
                      <span className="font-mono text-xs shrink-0">
                        <span className={`font-bold ${rosterComplete ? 'text-white' : 'text-white'}`}>{rosterCount}</span>
                  ...
                  <div className="flex items-stretch gap-2 sm:gap-2.5 ml-auto">
                    ...
                    <div className="flex items-center gap-2.5 px-3 py-2 rounded-lg bg-neonGreen/[0.06] border border-neonGreen/20 group-hover:border-neonGreen/30 transition-colors">
                      <div className="p-1.5 rounded-md bg-neonGreen/15 text-white shrink-0">
                        <Coins className="w-3.5 h-3.5" />
                      </div>
                      <div className="leading-tight">
                        <p className="text-[9px] font-bold uppercase tracking-wider text-mutedText">Remaining Purse</p>
                        <p className="font-mono text-xs sm:text-sm font-bold text-white mt-0.5">{formatCurrency(team.remainingBudget)}</p>
