...
export default function GeneralTournaments() {
  ...
  return (
    <div className="space-y-6">
      {/* Tournament hero */}
      <div className="relative overflow-hidden rounded-2xl border border-cardBorder bg-gradient-to-r from-warningGold/40 via-cardBg to-darkBg p-6 sm:p-8">
        <div className="absolute -top-16 -right-16 w-64 h-64 rounded-full bg-warningGold/10 blur-3xl pointer-events-none" />
        <div className="relative space-y-3">
          <p className="flex items-center gap-1.5 text-[11px] font-mono font-bold uppercase tracking-widest text-warningGold">
          ...
          <div className="flex flex-wrap gap-x-6 gap-y-1 pt-2 text-xs text-secondaryText">
            <span>Season: <strong className="text-primaryText">{sessions?.length || 0}</strong></span>
            <span>Franchises: <strong className="text-primaryText">{teams?.length || 0}</strong></span>
            <span>Total Matches: <strong className="text-primaryText">{processed.length}</strong></span>
          </div>
        </div>
      </div>
        ...
        {[
          ...
          return (
            <Link key={link.label} to={link.to}
              className="glass-card glass-card-hover rounded-2xl p-4 border border-borderStrong flex items-center gap-3 group">
              <Icon className={`w-5 h-5 ${link.accent}`} />
              <span className="text-xs font-bold text-primaryText group-hover:text-white">{link.label}</span>
              <ArrowRight className="w-3.5 h-3.5 text-mutedText ml-auto group-hover:text-secondaryText" />
            </Link>
          );
        })}
      </div>
      ...
      <section className="glass-card rounded-2xl p-5 border border-cardBorder space-y-4">
        <h3 className="flex items-center gap-2 text-xs font-bold uppercase tracking-wider text-secondaryText">
          <ShieldCheck className="w-4 h-4 text-warningGold" /> Participating Franchises
        </h3>
        {teams?.length ? (
          ...
          <p className="flex items-center gap-2 text-xs text-mutedText py-4 justify-center">
...
const PHASE_LABELS_FALLBACK = { label: 'Coming Soon', color: 'text-secondaryText bg-surfaceHover border-borderStrong' };
