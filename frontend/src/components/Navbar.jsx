...
import { Trophy, Shield, Gavel, Users, User, Radio, LogOut, CheckCircle2, AlertTriangle, ChevronRight } from 'lucide-react';
    ...
    <header className="sticky top-0 z-50 glass-card border-b border-slate-800 backdrop-blur-md">
      ...
      {lastActionToast && (
        <div className={`px-4 py-2 text-xs font-bold text-center flex items-center justify-center gap-2 transition-all ${
          ...
          {lastActionToast.type === 'error' ? <AlertTriangle className="w-4 h-4 text-rose-400 animate-bounce" /> : <CheckCircle2 className="w-4 h-4 text-emerald-400" />}
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          ...
          {/* Role Switcher Toolbar (For easy PRD evaluation & navigation) */}
          <div className="flex items-center space-x-2">
            <div className="hidden lg:flex items-center space-x-1 bg-slate-950/80 p-1 rounded-xl border border-slate-800 text-xs">
              <button
                onClick={() => handleRoleSelect('super_admin', '/admin/dashboard')}
                className={`flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg transition font-medium ${
                  location.pathname.startsWith('/admin') ? 'bg-blue-600 text-white shadow-md' : 'text-slate-400 hover:text-slate-200'
                }`}
              >
              ...
              <button
                onClick={() => handleRoleSelect('podium_admin', '/podium/dashboard')}
                className={`flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg transition font-medium ${
                  location.pathname.startsWith('/podium') ? 'bg-rose-600 text-white shadow-md' : 'text-slate-400 hover:text-slate-200'
                }`}
              >
              ...
              <button
                onClick={() => handleRoleSelect('manager', '/manager/dashboard')}
                className={`flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg transition font-medium ${
                  location.pathname.startsWith('/manager') ? 'bg-emerald-600 text-white shadow-md' : 'text-slate-400 hover:text-slate-200'
                }`}
              >
              ...
              <button
                onClick={() => handleRoleSelect('player', '/player/dashboard')}
                className={`flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg transition font-medium ${
                  location.pathname.startsWith('/player') ? 'bg-purple-600 text-white shadow-md' : 'text-slate-400 hover:text-slate-200'
                }`}
              >
              ...
              <Link
                to="/live"
                className={`flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg transition font-medium ${
                  location.pathname === '/live' ? 'bg-amber-600 text-white shadow-md' : 'text-slate-400 hover:text-slate-200'
                }`}
              >
